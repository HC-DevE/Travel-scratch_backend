...
const { TripPlace } = require('../models')(sequelize);
...
exports.createTripPlace = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ success: false, message: 'Failed to create tripPlace' });
  }
};
...
exports.getTripPlaces = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, message: 'Failed to get tripPlaces' });
  }
};
...
exports.getTripPlaceById = async (req, res) => {
  try {
    ...
    if (!tripPlace) {
      return res.status(404).json({ success: false, message: 'TripPlace not found' });
    }
    res.status(200).json({ success: true, tripPlace });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Failed to get tripPlace' });
  }
};
...
exports.updateTripPlace = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ success: false, message: 'Failed to update tripPlace' });
  }
};
...
exports.deleteTripPlace = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'TripPlace deleted successfully' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Failed to delete tripPlace' });
  }
};
